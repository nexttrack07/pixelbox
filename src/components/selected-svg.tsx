...
import { Element, selectedElementState, SvgElement } from "stores/element.store"
import { ColorResult, BlockPicker, Color } from "react-color"
      ...
      <div className="flex items-center space-x-8">
        <div>
          ...
          <div className="dropdown">
            <label tabIndex={0} className="btn w-20 h-18" style={{ backgroundColor: svgProps?.fill }} />
            <div tabIndex={0} className="dropdown-content">
              <BlockPicker onChangeComplete={handleFillChange} color={svgProps?.fill} />
            </div>
          </div>
        </div>
        <div>
          ...
          <div className="dropdown">
            <label tabIndex={0} className="btn w-20 h-18" style={{ backgroundColor: svgProps?.stroke }} />
            <div tabIndex={0} className="dropdown-content">
              <BlockPicker onChangeComplete={handleStrokeChange} color={svgProps?.stroke} />
            </div>
          </div>
        </div>
      </div>
      <Slider value={svgProps?.strokeWidth || 0} label="Stroke Width" />
    </div>
  )
}